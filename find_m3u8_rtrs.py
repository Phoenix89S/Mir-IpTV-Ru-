...
def detect_group(name: str) -> str:
    ...
    ЛОГИКА:
    1) Ищем совпадение по ПОДСТРОКЕ (как раньше).
       Это позволяет находить каналы даже если в плейлисте
       есть лишние слова, приставки, суффиксы и т.п.

    2) Но перед возвратом кнопки — проверяем WHITELIST.
       Если найденное имя НЕ входит в официальный список
       каналов кнопки — это мусор → отправляем в «Общие».

    3) Если не найдено ни одного совпадения — «Общие».
    """
    ...
    for button, mapping in CHANNEL_GROUPS.items():
        for _, channel_name in mapping.items():
            ...
            # 1) Совпадение по подстроке (как раньше)
            if channel_name.lower() in clean.lower():

                # 2) Проверка WHITELIST — не пускаем мусор
                if channel_name not in WHITELISTS[button]:
                    return "Общие"

                # 3) Канал официально разрешён → возвращаем кнопку
                return button

    # 4) Если не нашли ни в одной кнопке → в «Общие»
    return "Общие"
