    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        ...
        for orbit in sorted_orbits:
            for ch in rtrs_data[orbit]:
                counters[(orbit, ch["name"])] += 1
                idx = counters[(orbit, ch["name"])]
                disp = ch["name"] if idx == 1 else f'{ch["name"]} ({idx})'
                f.write(f'#EXTINF:-1 tvg-id="{ch["tvg_id"]}" group-title="{MAIN_GROUP_NAME}",Кнопка {orbit} {disp}\n{ch["link"]}\n')

        # Общие
        counters = defaultdict(int)
        for ch in all_channels[OTHER_GROUP_NAME]["999"]:
            counters[ch["name"]] += 1
            idx = counters[ch["name"]]
            disp = ch["name"] if idx == 1 else f'{ch["name"]} ({idx})'
            f.write(f'#EXTINF:-1 tvg-id="{ch["tvg_id"]}" group-title="{OTHER_GROUP_NAME}",{disp}\n{ch["link"]}\n')
