...
def detect_group(name: str) -> str:
    ...
    Ключевые правила:
    -----------------
    1) Линейка «СМОТРим» — отдельная кнопка.
       НЕ нормализуется до ОТР, НЕ смешивается с эфиркой.

    2) Эфирные каналы определяются по АВТОМАТИЧЕСКОМУ канону,
       который растёт вместе со словарём.

    3) Совпадение по подстроке выполняется между нормализованными именами.

    4) WHITELIST проверяется по нормализованному канону.

    5) Если совпадений нет — «Общие».
    """

    clean = normalize_name(name)

    # ==========================================
    # 0) Специальная логика для линейки СМОТРим
    # ==========================================
    if "смотрим" in clean.lower():
        return "СМОТРим"

    # ==========================================
    # 1) Эфирные ТВ Плюс (автоматический канон)
    # ==========================================
    for ether_name in ETHERS_CANONICAL:
        if ether_name.lower() in clean.lower():
            return "Эфирные ТВ Плюс"

    # ==========================================
    # 2) Общая логика определения кнопки
    # ==========================================
    for button, mapping in CHANNEL_GROUPS.items():
        for _, canonical in mapping.items():
            ...
            if canonical_norm.lower() in clean.lower():

                if canonical_norm not in WHITELISTS[button]:
                    return "Общие"

                return button

    # ==========================================
    # 3) Если не нашли ни в одной кнопке
    # ==========================================
    return "Общие"
